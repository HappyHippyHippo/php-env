...
if [ $STATE == "on" ]; then
  ...
  spinup_check_app_on $app $version auth-api $auth_api_version
  spinup_check_app_on $app $version org-api $org_api_version
  spinup_check_app_on $app $version project-api $project_api_version
  spinup_check_app_on $app $version feedback-api $feedback_api_version
  ...
  regex="s|<auth_api_version>|$auth_api_version|g;$regex"
  regex="s|<org_api_version>|$org_api_version|g;$regex"
  regex="s|<project_api_version>|$project_api_version|g;$regex"
  regex="s|<feedback_api_version>|$feedback_api_version|g;$regex"
