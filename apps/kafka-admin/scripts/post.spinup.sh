...
if [ $STATE == "on" ]; then
  ...
  sed "s|<app>|$app|g;s|<container>|$container|g;s|<port>|$port|g" \
    ...
    > $dir_data/reverse-proxy/vhosts/kafka/$app.conf
else
  # remove any proxy registration of the application
  if [ -f $(path $dir_data/reverse-proxy/vhosts/kafka/$app.conf) ]; then
    io_spinup_remove_from_reverse_proxy $app
    rm $dir_data/reverse-proxy/vhosts/kafka/$app.conf
  fi
fi
